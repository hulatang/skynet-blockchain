...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		...
		echo venv/bin/python -m pip install --force --no-binary skynetvdf "$SKYNETVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary skynetvdf "$SKYNETVDF_VERSION"
