...
from skynet.ssl.create_ssl import generate_ca_signed_cert, get_skynet_ca_crt_key, make_ca_cert
...
def create_all_ssl(root: Path):
    ...
    skynet_ca_crt_path.write_bytes(skynet_ca_crt)
    skynet_ca_key_path.write_bytes(skynet_ca_key)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if skynet_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return skynet_init(root_path)
...
def skynet_init(root_path: Path):
