...
- We have released version 1.0.0 of [chiapos](https://github.com/SkynetNetwork/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- This is a hard fork/breaking change from RC6/7. TXNT Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.skynet.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- We have moved to a new and much higher scalability download.skynet.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.skynet.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.skynet.net/latest/Setup-MacOS.dmg).
- Skynet wheels not on Pypi and some dependecies not found there also are now on pypi.chia.net.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.skynet.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- calccrypto/uint128_t was updated in the Windows chiapos implementation. Skynetpos required some changes its build process to support MacOS ARM64.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.skynet.net via https, for example.
...
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run skynet-blockchain.
...
- Partial support for Python 3.9. That includes new versions of Skynet dependencies like chiabip158.
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/SkynetNetwork/chiapos/releases/tag/0.12.32).
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Skynet Network at backup.skynet.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Skynet backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.skynet.net, your own installation, or a third party's version of it.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the skynet-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- There was a regression in Skynet Proof of Space ([chiapos](https://github.com/SkynetNetwork/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- The libraries skynet-pos, skynet-fast-vdf, and skynet-bip-158 have been moved to their own repositories: [chiapos](https://github.com/SkynetNetwork/chiapos), [chiavdf](https://github.com/SkynetNetwork/chiavdf), and [chaibip158](https://github.com/SkynetNetwork/chiabip158). They are brought in by skynet-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/SkynetNetwork/bls-signatures).
...
- More details on the release at [https://www.skynet.net/developer/](https://www.skynet.net/developer/)
